...
void inorderPrintInRange(Node root,int k1,int k2){
    if(root->left!=NULL)inorderPrintInRange(root->left,k1,k2);
    if(root->d>=k1 && root->d<=k2)printf("%d,",root->d);
    if(root->right!=NULL)inorderPrintInRange(root->right,k1,k2);
}
 
int main(){
    Node root=NULL;
    int n,temp;
    puts("Enter size of first BST then its elements");
    scanf("%d",&n);
    for(int i=0;i<n;i++){
        fflush(stdin);
        scanf("%d",&temp);
    ...
    puts("Enter the lower and upper limits");
    int k1,k2;
    scanf("%d",&k1);
    scanf("%d",&k2);
    puts("the values in given range are->>");
    inorderPrintInRange(root,k1,k2);
