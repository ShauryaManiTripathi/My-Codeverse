...
int main(){
    ...
    Node root2=NULL;
    int n2;
    puts("\nEnter size of second BST then its elements\n");
    scanf("%d",&n2);
    for(int i=0;i<n2;i++){
        fflush(stdin);
        scanf("%d",&temp);
        insertBST(&root2,temp);
    }
    DisplayLinkedTree(root2);
    puts("inorder->");
    inorder(root2);
    puts("");
    int a[n];
    int a2[n];
    inorderArrayFill(root,a);
    b=0;
    inorderArrayFill(root2,a2);
    b=0;
    if(n!=n2)puts("Similar BST->No");
    else{
        int i;
        for(i=0;i<n;i++){
            if(a[i]!=a2[i])break;
        }
        if(i==n)puts("SImilar BST->Yes");
        else puts("Similar BST->No");
    }
